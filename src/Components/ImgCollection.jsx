    ...
    for (let i = 0; i < imgCollection.length; i += 2) {
      collection.push(
        <div className="pt-5 flex flex-row justify-between items-center" key={i + "a"}>
          <p className="w-[47%] text-white text-2xl text-center" key={i + "b"} >{imgCollection[i][0]}</p>
          <p className="w-[47%] text-white text-2xl text-center" key={i + "c"}>{imgCollection[i + 1][0]}</p>
        </div >
      );
      collection.push(
